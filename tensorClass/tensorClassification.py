...
def getFrame(sec):
    cap.set(cv2.CAP_PROP_POS_MSEC,sec*100)
...
if __name__ == "__main__":
    ...
    while success:
        ...
        if count<0:
            count = 2
        cv2.putText(img,"Count {}".format(str(count-2)),(10,50), font, 0.75,(255,0,0),1,cv2.LINE_AA)
        cv2.imshow("preview", img)
        cv2.imwrite('./images/frame'+str(xyz)+'.jpg', img)
        sec = sec + frameRate
        sec = round(sec, 2)
        success = getFrame(sec)
        key = cv2.waitKey(1)
